...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  $sql = "SELECT * FROM job WHERE aangenomen_door=? OR bespannen_door=? OR uitgegeven_door=? ";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("iii", $id, $id, $id);
  $stmt->execute();
  $check = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
  if (empty($check)) {
    deleteItem($conn, $id, "medewerker");
  } else {
...
<head>
  ...
  <title>Medewerkers - BeeS Bespanlijst</title>
</head>
...
<body>
  <header class="position-fixed w-100">
    ...
    <div class="custom-20 d-custom-flex phone-none">
      ...
      <a href="medewerkers.php" class="custom-21 mr-3 text-decoration-none" style="color: black">medewerkers</a>
  ...
  <nav class="custom-4 bg-gradient-primary text-center phone-none" id="main-nav-bar">
    ...
    <a href="medewerkers.php"
      ...
      <div id="nav-eight" class=" d-custom-none phone-block text-white">medewerkers</div>
      <span class="custom-6 material-icons text-white">manage_accounts</span>
    </a>
    <a href="klanten.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
...
<script>
  ...
  function selectPage() {
    var medewerkers = <?php echo json_encode($medewerkers); ?>;
    ...
    if (pages[currentPage] == "PageOne") {
      ...
      getContentEmployees(medewerkers);
      pageTitle = "medewerkers";
      curPage = "btn-one";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageTwo") {
      content = "<div><form action='creatingemployee.php' method='post'><div class='content-container w-100 d-flex justify-content-center mt-3'><div class='phone-width-100 mx-1'>"
      content += "<p class='mx-1 mb-0 font-weight-bold text-dark'>medewerker aanmaken</p>"
      content += start;
      ...
      content += "<div><label class='d-block mx-1 mb-0'>" + "achternaam" + "</label><input class='form-input-small' maxlength='100 phone-width-100' type='text' name='" + "achternaam" + "' required/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "e-mail" + "</label><input class='form-input-big phone-width-100' maxlength='100' minlength='3' type='email' name='" + "email" + "' required/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "wachtwoord" + "</label><input class='form-input-big phone-width-100' type='password' name='" + "wachtwoord" + "' required/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "rol" + "</label><select class='form-input-big phone-width-100' required name='" + "rol" + "'>";
      for (var i = 0; i < roles.length; i++) {
        content += "<option  value='" + roles[i] + "'>" + roles[i] + "</option>";
      }
      content += "</select></div>";
      content += end;
  ...
  function getContentEmployees(array) {
    if (array) {
      for (var i = 0; i < array.length; i++) {
        content += "<a href='medewerker-weergeven.php?id=" + array[i].id + "' class='text-decoration-none'><article class='custom-19 d-custom-flex phone-block p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + truncateString(array[i].voornaam + " " + array[i].achternaam, 25) + "</p><div class='d-flex w-100'><p class='w-100 mb-0'>" + truncateString(array[i].email, 25) + "</p><p class='mb-0 phone-font-weight-bold'>" + array[i].rol + "</p></div></article></a>";
      }
    } else {
      content += "";
    }

...
// VARIABLES
$klant_types = getCustomerTypes($conn);
$klanten = getCustomers($conn);
...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  $sql = "SELECT * FROM klant WHERE klant_type=? ";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("i", $id);
  $stmt->execute();
  $check = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
  if (empty($check)) {
    deleteItem($conn, $id, "klant_type");
  } else {
...
<head>
  ...
  <title>Klanten - BeeS Bespanlijst</title>
</head>
...
<body>
  <header class="position-fixed w-100">
    ...
    <div class="custom-20 d-custom-flex phone-none">
      ...
      <a href="klanten.php" class="custom-21 mr-3 text-decoration-none" style="color: black">klanten</a>
...
<script>
  ...
  function selectPage() {
    var klanten = <?php echo json_encode($klanten); ?>;
    var klant_types = <?php echo json_encode($klant_types); ?>;
    ...
    if (pages[currentPage] == "PageOne") {
      ...
      getContentCustomers(klanten);
      pageTitle = "klanten";
      curPage = "btn-one";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageTwo") {
      content = "<div class='m-3'></div>";
      getContentCustomerTypes(klant_types);
      pageTitle = "opties";
      curPage = "btn-two";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageThree") {
      content = "<div><form action='creatingcustomertype.php' method='post'><div class='content-container w-100 d-flex justify-content-center mt-3'><div class='phone-width-100 mx-1'>"
      content += "<p class='mx-1 mb-0 font-weight-bold text-dark'>opdrachtgever aanmaken</p>"
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "klant type" + "</label><input class='form-input-big phone-width-100' type='text' required name='type'/></div>";
      content += end;
  ...
  function getContentCustomerTypes(array) {
    for (var i = 0; i < array.length; i++) {
      content += "<div><article class='custom-22 d-flex p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + array[i].type + "</p><div class='d-flex w-100'><p class='mb-0 phone-font-weight-bold w-100 d-flex justify-content-end'>" + "<a href='klanten.php?id=" + array[i].id + "&item=strength' class='text-decoration-none text-danger'>verwijderen</a>" + "</p></div></article></div>";
    }
  }

  function getContentCustomers(array) {
    if (array) {
      for (var i = 0; i < array.length; i++) {
        content += "<a href='klant-weergeven.php?id=" + array[i].id + "' class='text-decoration-none'><article class='custom-19 d-custom-flex phone-block p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + truncateString(array[i].voornaam + " " + array[i].achternaam, 25) + "</p><div class='d-flex w-100'><p class='w-100 mb-0'>" + truncateString(array[i].email, 25) + "</p><p class='mb-0 phone-font-weight-bold'>" + "" + "</p></div></article></a>";
      }
    } else {
      content += "";
    }

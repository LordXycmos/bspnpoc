...
// VARIABLES
$snaren = getStrings($conn);
$bespankrachten = getStrength($conn);
...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  if ($_GET['item'] == "strength") {
    $sql = "SELECT * FROM job WHERE snaar=? ";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $check = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    if (empty($check)) {
      deleteItem($conn, $id, "bespan_kracht");
    } else {
      header("location:snaren.php");
    }
  } else {
    $sql = "SELECT * FROM job WHERE kracht_lengte=? OR kracht_breedte=? ";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $id, $id);
    $stmt->execute();
    $check = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    if (empty($check)) {
      deleteItem($conn, $id, "snaren");
    } else {
      header("location:snaren.php");
    }
  }
...
<head>
  ...
  <title>Snaren - BeeS Bespanlijst</title>
</head>
...
<body>
  <header class="position-fixed w-100">
    ...
    <div class="custom-20 d-custom-flex phone-none">
      ...
      <a href="snaren.php" class="custom-21 mr-3 text-decoration-none" style="color: black">snaren</a>
  ...
  <nav class="custom-4 bg-gradient-primary text-center phone-none" id="main-nav-bar">
    ...
    <a href="snaren.php"
      ...
      <div id="nav-ten" class=" d-custom-none phone-block text-white">snaren</div>
      <span class="custom-6 material-icons text-white">manage_accounts</span>
    </a>
...
<script>
  ...
  function selectPage() {
    if (pages[currentPage] == "PageOne") {
      var snaren = <?php echo json_encode($snaren); ?>;
      content = "<div class='m-3'></div>";
      getContentStrings(snaren);
      pageTitle = "snaren";
      curPage = "btn-one";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageTwo") {
      var bespankrachten = <?php echo json_encode($bespankrachten); ?>;
      content = "<div class='m-3'></div>";
      getContentStrengths(bespankrachten);
      pageTitle = "bespankrachten";
      curPage = "btn-two";
      pageDisplay(content, pageTitle, curPage);
      ...
      var end = "</div>";
      // SNAAR AANMAKEN
      content = "<div><form action='creatingstring.php' method='post'><div class='content-container w-100 d-flex justify-content-center mt-3'><div class='phone-width-100 mx-1'>"
      content += "<p class='mx-1 mb-0 font-weight-bold text-dark'>snaar aanmaken</p>"
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "type snaar" + "</label><input class='form-input-big phone-width-100' type='text' required name='type_snaar'/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "prijs" + "</label><input class='form-input-big phone-width-100' type='number' step='0.01' min='0' required name='prijs'/></div>";
      content += end;
      content += "<div class='d-flex mt-3'><input class='btn btn-outline-dark w-100 phone-mx-1' type='submit' required value='+' />";
      content += "</div></div></div></form><div>"

      // BESPANKRACHT AANMAKEN
      ...
      content += "<p class='mx-1 mb-0 font-weight-bold text-dark'>bespankracht aanmaken</p>"
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "bespankracht" + "</label><input class='form-input-big phone-width-100' type='number' step='0.1' min='0' required name='bespan_kracht'/></div>";
      content += end;
  ...
  function getContentStrings(array) {
    for (var i = 0; i < array.length; i++) {
      content += "<div><article class='custom-22 d-custom-flex phone-block p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + array[i].type + "</p><div class='d-flex w-100'><p class='w-100 mb-0'>" + "€" + array[i].prijs + "</p><p class='mb-0 phone-font-weight-bold'>" + "<a href='snaren.php?id=" + array[i].id + "&item=string' class='text-decoration-none text-danger'>verwijderen</a>" + "</p></div></article></a>";
    }
  }

  function getContentStrengths(array) {
    array.sort(function (a, b) {
      return a.bespankracht - b.bespankracht;
    });
    for (var i = 0; i < array.length; i++) {
      content += "<div><article class='custom-22 d-flex p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + array[i].bespankracht + "</p><div class='d-flex w-100'><p class='mb-0 phone-font-weight-bold w-100 d-flex justify-content-end'>" + "<a href='snaren.php?id=" + array[i].id + "&item=strength' class='text-decoration-none text-danger'>verwijderen</a>" + "</p></div></article></div>";
    }
  }
</script>
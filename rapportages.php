...
// VARIABLES
$totalAangenomen = getAangenomenCount($conn);
$totalBespannen = getBespannenCount($conn);
$totalUitgegeven = getUitgegevenCount($conn);



// FUNCTIONS
function getAangenomenCount($conn)
{
  $sql = "SELECT * FROM job WHERE aangenomen_door IS NOT NULL";
  $stmt = $conn->prepare($sql);
  $stmt->execute();
  return $stmt->get_result()->num_rows;
}

function getBespannenCount($conn)
{
  $sql = "SELECT * FROM job WHERE bespannen_door IS NOT NULL";
  $stmt = $conn->prepare($sql);
  $stmt->execute();
  return $stmt->get_result()->num_rows;
}

function getUitgegevenCount($conn)
{
  $sql = "SELECT * FROM job WHERE uitgegeven_door IS NOT NULL";
  $stmt = $conn->prepare($sql);
  $stmt->execute();
  return $stmt->get_result()->num_rows;
}
...
<head>
  ...
  <title>Rapportages - BeeS Bespanlijst</title>
</head>
...
<body>
  <header class="position-fixed w-100">
    ...
    <div class="custom-20 d-custom-flex phone-none">
      <a href="rapportages.php" class="custom-21 mr-3 text-decoration-none" style="color: black">rapportages</a>
  ...
  <nav class="custom-4 bg-gradient-primary text-center phone-none" id="main-nav-bar">
    ...
    <a href="rapportages.php"
      ...
      <div id="nav-six" class=" d-custom-none phone-block text-white">rapportages</div>
      <span class="custom-6 material-icons text-white">manage_accounts</span>
    </a>

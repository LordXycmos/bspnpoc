...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  $sql = "SELECT * FROM job WHERE apparaat=? ";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("i", $id);
  $stmt->execute();
  $check = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
  if (empty($check)) {
    deleteItem($conn, $id, "apparaten");
  } else {
...
<head>
  ...
  <title>Apparaten - BeeS Bespanlijst</title>
</head>
...
<body>
  <header class="position-fixed w-100">
    ...
    <div class="custom-20 d-custom-flex phone-none">
      ...
      <a href="apparaten.php" class="custom-21 mr-3 text-decoration-none" style="color: black">apparaten</a>
    </div>
  </header>
  <div
  ...
  <nav class="custom-4 bg-gradient-primary text-center phone-none" id="main-nav-bar">
    ...
    <a href="apparaten.php"
      ...
      <div id="nav-eleven" class=" d-custom-none phone-block text-white">apparaten</div>
      <span class="custom-6 material-icons text-white">manage_accounts</span>
    </a>
...
<script>
  ...
  function selectPage() {
    ...
    if (pages[currentPage] == "PageOne") {
      ...
      getContentMachines(apparaten);
      pageTitle = "apparaten";
      curPage = "btn-one";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageTwo") {
      content = "<div><form action='creatingmachine.php' method='post'><div class='content-container w-100 d-flex justify-content-center mt-3'><div class='phone-width-100 mx-1'>"
      content += "<p class='mx-1 mb-0 font-weight-bold text-dark'>apparaat aanmaken</p>"
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "apparaat naam" + "</label><input class='form-input-big phone-width-100' type='text' required name='naam'/></div>";
      content += end;
  ...
  function getContentMachines(array) {
    for (var i = 0; i < array.length; i++) {
      content += "<div><article class='custom-22 d-flex p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + truncateString(array[i].naam, 25) + "</p><div class='d-flex w-100'><p class='mb-0 phone-font-weight-bold w-100 d-flex justify-content-end'>" + "<a href='apparaten.php?id=" + array[i].id + "&item=strength' class='text-decoration-none text-danger'>verwijderen</a>" + "</p></div></article></div>";
    }
  }

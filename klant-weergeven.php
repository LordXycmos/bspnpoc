...
// VARIABLES
$customer = getCustomer($id, $conn);
$customerType = getCustomerType($customer['klant_type'], $conn);
$customerTypes = getCustomerTypes($conn);
...
<script>
  ...
  function selectPage() {
    const klant = <?php echo json_encode($customer); ?>;
    const klant_type = <?php echo json_encode($customerType); ?>;
    const klant_types = <?php echo json_encode($customerTypes); ?>;
    var start = "<div class='d-custom-flex phone-block mt-2'>";
    var end = "</div>";
    names = ["voornaam", "achternaam", "email", "klant type", "aanmaak datum"];
    stats = [klant['voornaam'], klant['achternaam'], klant['email'], klant_type['type'], klant['aanmaak_datum']];
    if (pages[currentPage] == "PageOne") {
      ...
      content = "<form action='editcustomer.php?id=" + <?php echo $id ?> + "' method='post'><div class='content-container w-100 d-flex justify-content-center mt-3'><div class='phone-width-100 mx-1'>"
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "voornaam" + "</label><input class='form-input-small phone-width-100' required type='text' name='" + "voornaam" + "' value='" + klant["voornaam"] + "'/></div>";
      content += "<div><label class='d-block mx-1 mb-0'>" + "achternaam" + "</label><input class='form-input-small phone-width-100' required type='text' name='" + "achternaam" + "' value='" + klant["achternaam"] + "'/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "e-mail" + "</label><input class='form-input-big phone-width-100' type='email' required name='" + "email" + "' value='" + klant["email"] + "'/></div>";
      content += end;
      content += start;
      content += "<div><label class='d-block mx-1 mb-0'>" + "rol" + "</label><select class='form-input-big phone-width-100' required name='" + "klant_type" + "'>";
      for (var i = 0; i < klant_types.length; i++) {
        if (klant_type['id'] == klant_types[i].id) {
          selected = " selected ";
        } else {
          selected = "";
        }
        content += "<option" + selected + "  value='" + klant_types[i].id + "'>" + klant_types[i].type + "</option>";
      }
      content += "</select></div>";
      content += end;

...
$jobUnpaid = getUnpaidJobs($id, $conn);
$jobPaid = getPaidJobs($id, $conn, $curMonth, $prevMonth);
...
// FUNCTIONS
function getUnpaidJobs(int $id, $conn)
{
  $sql = "SELECT * FROM job WHERE bespannen_door=? AND status=3 OR bespannen_door=? AND status=2";
...
function getPaidJobs(int $id, $conn, $curMonth, $prevMonth)
{
  $sql = "SELECT * FROM job WHERE bespannen_door=? AND status=4 AND uitbetaal_maand=? OR bespannen_door=? AND status=4 AND uitbetaal_maand=?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("isis", $id, $curMonth, $id, $prevMonth);
  $stmt->execute();
  return $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
}
...
<head>
  ...
  <title>Salaris - BeeS Bespanlijst</title>
</head>
...
<body>
  ...
  <nav class="custom-4 bg-gradient-primary text-center phone-none" id="main-nav-bar">
    ...
    <a href="salaris.php"
      ...
      <div id="nav-four" class=" text-white d-custom-none phone-block">salaris</div>
      <span class="custom-6 material-icons text-white">payments</span>
    </a>
    ...
    <a href="rapportages.php"
      ...
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">rapportages</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
    ...
    <a href="manager.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">uitbetalen</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
    <a href="manager.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">medewerkers</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
    <a href="manager.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">klanten</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
    <a href="manager.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">snaren</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
    <a href="manager.php"
      class="custom-5 d-custom-none text-decoration-none justify-center text-center align-items-center p-3 phone-flex"
      id="nav-manager" <?php echo $managerButton; ?>>
      <div id="nav-five" class=" d-custom-none phone-block" style="color: #8e8e8e">apparaten</div>
      <span class="custom-6 material-icons">manage_accounts</span>
    </a>
...
<script>
  ...
  function selectPage() {
    if (pages[currentPage] == "PageOne") {
      ...
      content = "<p class='m-3 font-weight-bold text-dark'>te goed</p>"
      getContent(jobUnpaid);
      pageTitle = "open";
      curPage = "btn-one";
      pageDisplay(content, pageTitle, curPage);
    } else if (pages[currentPage] == "PageTwo") {
      var jobPaid = <?php echo json_encode($jobPaid); ?>;
      content = "<p class='m-3 font-weight-bold text-dark'>afgelopen maand</p>"
      getContent(jobPaid);
      pageTitle = "gesloten";
      curPage = "btn-two";
      pageDisplay(content, pageTitle, curPage);
  ...
  function getContent(array) {
    ...
    if (array) {
      for (var i = 0; i < array.length; i++) {
        ...
        content += "<a href='overzicht-weergeven.php?id=" + array[i].id + "' class='text-decoration-none'><article class='custom-19 d-custom-flex phone-block p-3 my-1 mx-3 text-muted text-decoration-none'><p class='w-100 mb-0 phone-text-dark'>" + resultCheck(truncateString(email.email, 28)) + "</p><div class='d-flex w-100'><p class='w-100 mb-0'>" + "€" + uitbetaal_methode.geld + " " + uitbetaal_methode.type + "</p><p class='mb-0 phone-font-weight-bold'>" + resultCheck(array[i].volgnummer) + "</p></div></article></a>";
      }
    } else {
      content += "";
    }

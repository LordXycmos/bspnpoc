<?php
// BASICS
session_start();
include_once 'dbconnection.php';

if (!isset ($_SESSION['id']) && is_numeric($_GET['id'])) {
  header('location:login.php');
}

if (isset ($_GET['id'])) {
  $id = $_GET['id'];
} else {
  header("Location: profiel.php");
}

checkExistence($job_id, $conn);



// VARIABLES
$voornaam = ucfirst(strtolower($_POST['voornaam']));
$achternaam = ucfirst(strtolower($_POST['achternaam']));
$email = $_POST['email'];
$password = $_POST['password'];
$wachtwoord = password_hash($password, PASSWORD_DEFAULT);



// GENERAL
editEmployee($conn, $voornaam, $achternaam, $email, $wachtwoord, $id);

$conn->close();
header("Location: profiel.php");
exit();



// FUNCTIONS
function checkExistence($id, $conn)
{
  $sql = "SELECT * FROM medewerker WHERE id=?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("i", $id);
  $stmt->execute();
  $result = $stmt->get_result()->fetch_assoc();
  if (empty ($result)) {
    header("Location: profiel.php");
  }
}

function editEmployee($conn, $voornaam, $achternaam, $email, $wachtwoord, $id)
{
  $stmt = $conn->prepare("UPDATE medewerker SET voornaam=?, achternaam=?, email=?, wachtwoord=? WHERE id=?");
  $stmt->bind_param("ssssi", $voornaam, $achternaam, $email, $wachtwoord, $id);
  $stmt->execute();
  $stmt->close();
}

function getRole(int $id, $conn)
{
  $sql = "SELECT rol FROM medewerker WHERE id=?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("i", $id);
  $stmt->execute();
  return $stmt->get_result()->fetch_assoc();
}